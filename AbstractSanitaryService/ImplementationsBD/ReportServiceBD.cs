    ...
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveItemPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var table = document.Tables.Add(rangeTable, items.Count, 2, ref missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = items[i].ItemName;
                    table.Cell(i + 1, 2).Range.Text = items[i].Price.ToString();
                }
        ...
        public List<WarehousesLoadViewModel> GetWarehousesLoad()
        {
            return context.Warehouses
                            .ToList()
                            .GroupJoin(
                                    context.WarehouseParts
                                                .Include(r => r.Part)
                                                .ToList(),
                                    warehouse => warehouse,
                                    warehousePart => warehousePart.Warehouse,
                                    (warehouse, warehouseParList) =>
        ...
        public List<CustomerOrderingsViewModel> GetCustomerOrderings(ReportBindingModel model)
        {
            return context.Orderings
                            .Include(rec => rec.Customer)
                            .Include(rec => rec.Item)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new CustomerOrderingsViewModel
                            {
        ...
        public void SaveCustomerOrderings(ReportBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetCustomerOrderings(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].ItemName, fontForCells));
                table.AddCell(cell);
