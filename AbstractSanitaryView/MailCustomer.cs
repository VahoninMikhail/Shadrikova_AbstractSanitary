...
namespace AbstractSanitaryView
{
    public static class MailCustomer
    {
        ...
        private static void GetMessages(int number)
        {
            ...
            string orderingSubjectMessage = string.Empty;
            string orderingBodyMessage = string.Empty;
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    {
                        orderingSubjectMessage = GetSubject(ref response, ref coding);

                        orderingBodyMessage = GetBody(response, coding);
                    }
                    if (!string.IsNullOrEmpty(messageId) && !string.IsNullOrEmpty(from) &&
                        !string.IsNullOrEmpty(orderingSubjectMessage) && !string.IsNullOrEmpty(date))
                    {
                        APIClient.PostRequestData("api/MessageInfo/AddElement",
                            new MessageInfoBindingModel
                            {
                                ...
                                Subject = orderingSubjectMessage,
                                Body = orderingBodyMessage
                            });

                        messageId = string.Empty;
                        from = string.Empty;
                        date = string.Empty;
                        orderingSubjectMessage = string.Empty;
                        orderingBodyMessage = string.Empty;
                    }
                }
            }
        }
